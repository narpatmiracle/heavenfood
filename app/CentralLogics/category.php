...
class CategoryLogic
{
    ...
    public static function restaurants(array $additional_data)
    {
        ...
        ->when($additional_data['avg_rating'] > 0 , function($query) use($additional_data) {
            $query->whereHas('reviews',function($q) use($additional_data){
                $q->selectRaw('avg(reviews.rating) as avg_r')
                    ->having('avg_r', '>=', $additional_data['avg_rating'])->groupby('food.restaurant_id');
            });
        })
        ...
        ->when($additional_data['top_rated'] == 1 , function($query){
                    $query->whereHas('reviews',function($q){
                        $q->selectRaw('avg(reviews.rating) as avg_r')
                            ->having('avg_r', '>=', 4)->groupby('food.restaurant_id');
                    });
                })
