...
class LoginController extends Controller
{
    ...
    public function login($login_url)
    {
        ...
        $data = array_column(DataSetting::whereIn('key', ['restaurant_employee_login_url', 'restaurant_login_url', 'admin_employee_login_url', 'admin_login_url'])->get(['key', 'value'])->toArray(), 'value', 'key');
        ...
        // Debugging the role to see if it's correctly determined

        // These lines are causing issues because dd() terminates execution
        $site_direction = $siteDirections[$role];
        $locale = $locals[$role];
        App::setLocale($locale);

        $captcha_builder = new CaptchaBuilder;
        $captcha_builder->setPhrase(strval(random_int(100000, 999999))); // Generate a random six-digit number
        $captcha_phrase = $captcha_builder->getPhrase();
        Session::get('six_captcha', $captcha_phrase);
        ...
        return view('auth.login', compact('captcha_phrase', 'email', 'password', 'role', 'site_direction', 'locale'));
